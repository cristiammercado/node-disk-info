    ...
    Darwin.run = function () {
        ...
        lines.forEach(function (value, index, array) {
            if (value !== '') {
                ...
                var d = new drive_1.default(tokens[0], isNaN(parseFloat(tokens[1])) ? 0 : +tokens[1], isNaN(parseFloat(tokens[2])) ? 0 : +tokens[2], isNaN(parseFloat(tokens[3])) ? 0 : +tokens[3], tokens[4], tokens.slice(5).join(' ')); // Prevent space splitting break the disk name
                drives.push(d);
            }
        });
        return drives;
